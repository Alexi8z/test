...
# NSFW keyword list for prompt-based filtering
NSFW_KEYWORDS = [
...
def check_safety(x_image, prompt="", disable_safety=False, sensitivity=0.5):
    ...
    # Combine model output and keyword check
    has_nsfw_concept = [hc or prompt_flag for hc in has_nsfw_concept]
...
def censor_batch(x, prompt="", disable_safety=False, sensitivity=0.5, replacement="blur"):
    ...
    for i, flag in enumerate(nsfw_flags):
        if flag:
            if replacement == "blur":
                img = (x_checked[i]*255).clip(0,255).astype(np.uint8)
                img = Image.fromarray(img)
                blurred = img.filter(ImageFilter.GaussianBlur(radius=40))  # strong blur
                ...
                # Bold text by drawing multiple offsets
